...
Vuvm_pkg::Vuvm_pkg(VerilatedContext* _vcontextp__, const char* _vcname__)
    : VerilatedModel{*_vcontextp__}
    , vlSymsp{new Vuvm_pkg__Syms(contextp(), _vcname__, this)}
...
Vuvm_pkg::Vuvm_pkg(const char* _vcname__)
    : Vuvm_pkg(Verilated::threadContextp(), _vcname__)
{
}
...
#endif  // VL_DEBUG
void Vuvm_pkg___024root___eval_static(Vuvm_pkg___024root* vlSelf);
void Vuvm_pkg___024root___eval_initial(Vuvm_pkg___024root* vlSelf);
void Vuvm_pkg___024root___eval_settle(Vuvm_pkg___024root* vlSelf);
void Vuvm_pkg___024root___eval(Vuvm_pkg___024root* vlSelf);

void Vuvm_pkg::eval_step() {
    VL_DEBUG_IF(VL_DBG_MSGF("+++++TOP Evaluate Vuvm_pkg::eval_step\n"); );
#ifdef VL_DEBUG
    // Debug assertions
    Vuvm_pkg___024root___eval_debug_assertions(&(vlSymsp->TOP));
#endif  // VL_DEBUG
    vlSymsp->clearTriggeredEvents();
    vlSymsp->__Vm_deleter.deleteAll();
    if (VL_UNLIKELY(!vlSymsp->__Vm_didInit)) {
        VL_DEBUG_IF(VL_DBG_MSGF("+ Initial\n"););
        Vuvm_pkg___024root___eval_static(&(vlSymsp->TOP));
        Vuvm_pkg___024root___eval_initial(&(vlSymsp->TOP));
        Vuvm_pkg___024root___eval_settle(&(vlSymsp->TOP));
        vlSymsp->__Vm_didInit = true;
    }
    VL_DEBUG_IF(VL_DBG_MSGF("+ Eval\n"););
    Vuvm_pkg___024root___eval(&(vlSymsp->TOP));
    // Evaluate cleanup
...
//============================================================
// Events and timing
bool Vuvm_pkg::eventsPending() { return !vlSymsp->TOP.__VdlySched.empty() && !contextp()->gotFinish(); }

uint64_t Vuvm_pkg::nextTimeSlot() { return vlSymsp->TOP.__VdlySched.nextTimeSlot(); }

//============================================================
// Utilities

const char* Vuvm_pkg::name() const {
    return vlSymsp->name();
}
...
void Vuvm_pkg___024root___eval_final(Vuvm_pkg___024root* vlSelf);

VL_ATTR_COLD void Vuvm_pkg::final() {
    contextp()->executingFinal(true);
    Vuvm_pkg___024root___eval_final(&(vlSymsp->TOP));
...
const char* Vuvm_pkg::hierName() const { return vlSymsp->name(); }
const char* Vuvm_pkg::modelName() const { return "Vuvm_pkg"; }
unsigned Vuvm_pkg::threads() const { return 1; }
void Vuvm_pkg::prepareClone() const { contextp()->prepareClone(); }
void Vuvm_pkg::atClone() const {
